...
sys.path.append('/home/akharchevnikova/Face_Image_Quality_Assessment/libsvm')
...
# SVM_MODEL_PATH = "C:\\akharche\\MasterThesis\\KeyFrameExtraction\\models\\allmodel"

# SVM_MODEL_PATH = "/home/student/akharchevnikova/models/allmodel"
SVM_MODEL_PATH = "/home/akharchevnikova/Face_Image_Quality_Assessment/models/allmodel"
...
def measure_brisque(img, load_image=True):
    if load_image:
        # read image from given path
        dis = cv2.imread(img, 1)
        if (dis is None):
    ...
    # load model
    model = svmutil.svm_load_model(SVM_MODEL_PATH)
...
def calculate_video(video_frames, k_best=5):
    brisque_measures = {}

    for frame_id, frame_path in video_frames.items():
        qualityscore = measure_brisque(frame_path, load_image=False)
        brisque_measures[frame_id] = qualityscore

    # Return k-best frames
    sorted_frames_scores = sorted(brisque_measures.items(), key=lambda x: x[1], reverse=True)

    return sorted_frames_scores[:k_best]
